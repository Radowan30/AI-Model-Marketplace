...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  useEffect(() => {
    const checkSessionExpiry = async () => {
      ...
      if (sessionStartTime && user) {
        ...
        const maxSessionTime = rememberMe
          ? 7 * 24 * 60 * 60 * 1000
          : 8 * 60 * 60 * 1000; // 7 days vs 8 hours
      ...
      if (rolesError) {
        ...
        if (roleNames.length === 0) {
          ...
          if (isRegistering === "true" || isLoggingIn === "true") {
            ...
            const registrationStartTime = parseInt(
              localStorage.getItem("registrationStartTime") || "0",
            );
            const loginStartTime = parseInt(
              localStorage.getItem("loginStartTime") || "0",
            );
            ...
            if (elapsedTime < 10000) {
              console.log(
                "Registration or login in progress, skipping auto-signout",
              );
              setLoading(false);
              return;
            } else {
  ...
  const switchRole = (newRole: "buyer" | "publisher") => {
    if (userRoles.includes(newRole)) {
      ...
      window.location.href =
        newRole === "publisher" ? "/publisher/dashboard" : "/buyer/dashboard";
    }
  };
